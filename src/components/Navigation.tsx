...
import { Compass, Menu, X, Home, Crown, LogIn, LogOut, ChevronDown, BookOpen, Heart, MessageCircle, Clock, Share2, Flame, Trophy } from 'lucide-react';
...
const Navigation: React.FC<NavigationProps> = ({
  ...
  const { user, profile, signOut, loading, isTrial, trialDaysRemaining } = useAuth();
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center space-x-4">
            <button onClick={() => onNavigate('home')} className={`${currentSection === 'home' ? 'text-cyan-600 font-semibold' : 'text-gray-600'} hover:text-cyan-600 transition-colors flex items-center gap-1 px-3 py-2`}>
              <Home className="w-4 h-4" />
              Início
            </button>
            <button onClick={() => onNavigate('browse')} className={`${currentSection === 'browse' ? 'text-cyan-600 font-semibold' : 'text-gray-600'} hover:text-cyan-600 transition-colors flex items-center gap-1 px-3 py-2`}>
              <Compass className="w-4 h-4" />
              Explorar
            </button>
            <button onClick={() => onNavigate('favorites')} className={`${currentSection === 'favorites' ? 'text-rose-600 font-semibold' : 'text-gray-600'} hover:text-rose-600 transition-colors flex items-center gap-1 px-3 py-2 relative`}>
            ...
            <button onClick={() => onNavigate('journal')} className={`${currentSection === 'journal' ? 'text-cyan-600 font-semibold' : 'text-gray-600'} hover:text-cyan-600 transition-colors flex items-center gap-1 px-3 py-2`}>
              <BookOpen className="w-4 h-4" />
              Diário
              {isPremium && !isTrial && (
                <span className="ml-1 px-1.5 py-0.5 bg-amber-100 text-amber-700 text-xs rounded-full">Premium</span>
              )}
              {isTrial && (
            ...
            {/* Shares/Gamification Button */}
            <button 
              onClick={() => onNavigate('shares')} 
              className={`${currentSection === 'shares' ? 'text-cyan-600 font-semibold' : 'text-gray-600'} hover:text-cyan-600 transition-colors flex items-center gap-1 px-3 py-2 relative`}
            >
            ...
            <Link to="/contato" className="text-gray-600 hover:text-cyan-600 transition-colors flex items-center gap-1 px-3 py-2">
              <MessageCircle className="w-4 h-4" />
              Contato
            </Link>
            
            {/* Trial indicator in nav */}
            ...
            {!isPremium && !isTrial && (
              <button onClick={() => onNavigate('subscription')} className="bg-gradient-to-r from-cyan-500 to-amber-500 text-white px-4 py-2 rounded-full text-sm font-medium hover:opacity-90 transition-opacity flex items-center gap-1">
                  ...
                  <>
                    ...
                    <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-lg border border-gray-100 py-2 z-20">
                      ...
                      <div className="py-1">
                        ...
                        <button
                          onClick={() => { onNavigate('shares'); setUserMenuOpen(false); }}
                          className="w-full flex items-center gap-2 px-4 py-2 text-gray-700 hover:bg-gray-50"
                        >
                          <Share2 className="w-4 h-4" />
                          Meu Impacto
                        ...
                        <button
                          onClick={() => { onNavigate('journal'); setUserMenuOpen(false); }}
                          className="w-full flex items-center gap-2 px-4 py-2 text-gray-700 hover:bg-gray-50"
                        >
                          <BookOpen className="w-4 h-4" />
                          Meu Diário
                        ...
                        <button
                          onClick={() => { onNavigate('subscription'); setUserMenuOpen(false); }}
                          className="w-full flex items-center gap-2 px-4 py-2 text-gray-700 hover:bg-gray-50"
                        >
                          <Crown className="w-4 h-4" />
          ...
          <div className="md:hidden py-4 border-t border-cyan-100">
            <div className="flex flex-col space-y-2">
              ...
              {isTrial && trialDaysRemaining !== null && (
                <div className="flex items-center gap-2 px-4 py-3 bg-purple-50 rounded-lg mb-2">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-purple-600">{trialDaysRemaining} {trialDaysRemaining === 1 ? 'dia restante' : 'dias restantes'}</p>
                  </div>
                </div>
              )}
              ...
              <button 
                ...
                className={`flex items-center gap-2 px-4 py-3 rounded-lg ${currentSection === 'home' ? 'bg-cyan-50 text-cyan-600' : 'text-gray-600 hover:bg-gray-50'}`}
              >
              ...
              <button 
                ...
                className={`flex items-center gap-2 px-4 py-3 rounded-lg ${currentSection === 'browse' ? 'bg-cyan-50 text-cyan-600' : 'text-gray-600 hover:bg-gray-50'}`}
              >
              ...
              <button 
                ...
                className={`flex items-center gap-2 px-4 py-3 rounded-lg ${currentSection === 'favorites' ? 'bg-rose-50 text-rose-600' : 'text-gray-600 hover:bg-gray-50'}`}
              >
              ...
              <button 
                onClick={() => { onNavigate('shares'); setMenuOpen(false); }} 
                className={`flex items-center gap-2 px-4 py-3 rounded-lg ${currentSection === 'shares' ? 'bg-cyan-50 text-cyan-600' : 'text-gray-600 hover:bg-gray-50'}`}
              >
              ...
              <button 
                onClick={() => { onNavigate('journal'); setMenuOpen(false); }} 
                className={`flex items-center gap-2 px-4 py-3 rounded-lg ${currentSection === 'journal' ? 'bg-cyan-50 text-cyan-600' : 'text-gray-600 hover:bg-gray-50'}`}
              >
                <BookOpen className="w-5 h-5" />
                Diário de Reflexões
                {isPremium && !isTrial && (
                  <span className="ml-auto px-2 py-0.5 bg-amber-100 text-amber-700 text-xs rounded-full">Premium</span>
                )}
                {isTrial && (
                  <span className="ml-auto px-2 py-0.5 bg-purple-100 text-purple-700 text-xs rounded-full">Teste</span>
                )}
              </button>
              <Link 
                to="/contato"
